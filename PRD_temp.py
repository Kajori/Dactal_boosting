...
#   DATE :  27/Nov/2013
#   VERSION :  v0.1
#   AUTHOR(s) :  J. Alvarez-Jarreta, modified by Kajori
#
#-------------------------------------------------------------------------------
	...
	decomposition = []
        for subset_str in subset_list :
            ...
            if ( (len(subset_list) > 1) and (len(subset) > self._subset) ) :
                ...
                print " tree =",(tree.as_string(schema="newick"))
                print " subset = ",subset
                print " len(subset) = ",len(subset)
                print " taxons in t = ", [t.taxon.label for t in tree.leaf_nodes() ]
                print " number of taxa t = ", len([t.taxon.label for t in tree.leaf_nodes() ])
