...
@author: smirarab, modified by Kajori for making it compatible with Dendropy version 4
'''
import dendropy
import sys
    ...
    for tree in trees:        
        ...
        if outgroups[0] == "-m":            
            ...
            if mrca.parent_node is None:
                ...
                if ingroup.edge.length is not None:
                    tree.reroot_at_edge(mrca.edge,  update_bipartitions=False,length1=mrca.edge.length/2,length2=mrca.edge.length/2)        
                else:
            ...
            if add_dummy:
                ...
                tree.reroot_at_edge(dummy.edge, update_bipartitions=False)
